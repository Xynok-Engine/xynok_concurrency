# `lane_queue` - hàng đợi chung của một lane

Tài liệu này giải thích **lane queue**: mảnh còn thiếu giữa các ring local trong
...
| [2](#2-lanequeue-là-gì-và-không-là-gì) | `LaneQueue` là gì, và quan trọng hơn: nó **không** là gì |
...
| [5](#5-fairness-vì-sao-phải-ép-worker-ngó-lane-queue) | Fairness: vì sao phải **ép** worker ngó nó |
...
## 2. `LaneQueue` là gì, và không là gì

**Lane queue là một hàng đợi MPMC không giới hạn, dùng chung cho cả lane, nằm cạnh các ring local.**

```text
                        ┌─────────────────────────────┐
   main thread ────────▶│         LANE QUEUE          │  nhiều người ghi
...
Chỗ dễ hiểu nhầm nhất, nên nói thẳng ra: **lane queue không thay thế ring local.**

Nó là tầng chậm, và nó phải chậm. Ring local mới là nơi phần lớn job đi qua, và mỗi lần một job
phải vòng qua lane queue là một lần bạn trả giá tranh chấp. Một pool khoẻ mạnh có lane queue
gần như lúc nào cũng rỗng: job rơi vào rồi bị hút lên ring local ngay.

Nếu bạn đo thấy lane queue lúc nào cũng đầy, đó không phải lane queue hoạt động tốt. Đó là dấu
hiệu ring local quá nhỏ hoặc worker đang bị bỏ đói.
...
pub fn spawn(&self, job: Job)
{
    match Worker::current()          // thread này có phải worker của pool không?
    {
        ...
        None => self.lane_queue.push(job),  // không: rơi vào lane queue
    }
}
```

Nhánh `Some` là lý do work-stealing nhanh: job do job sinh ra không bao giờ chạm lane queue.
...
fn push_local(&mut self, job: Job)
{
    ...
    // ring đầy: rút nửa cũ ra, ném xuống lane queue, giờ chắc chắn có chỗ
...
sở hữu ring nào của lane `Frame`, nên nó đi qua lane queue của lane đó. Cùng cơ chế với đường vào 1.

### Đường ra: lấy theo cụm, không lấy từng cái

```rust
// lấy nhiều, giữ 1 chạy ngay, phần còn lại nhét vào ring local
let job = lane_queue.steal_batch_and_pop(&mut self.worker, BATCH);
```

Lý do bắt buộc phải batch: lane queue là điểm dùng chung của mọi thread trong lane. Lấy một job
mỗi lượt nghĩa là mỗi job phải trả một lần tranh chấp trên cùng một cache line. Lấy 32 cái thì
bạn trả giá một lần, rồi 31 job sau chạy từ ring local và không đụng vào ai.

Kích thước cụm thường lấy `min(lane_queue.len() / n_workers + 1, ring.capacity() / 2)`. Chia cho
số worker để không có một thằng hốt sạch rồi những thằng khác vẫn đói.

---

## 4. Chỗ nó đứng trong vòng lặp worker

Lane queue xuất hiện đúng hai lần trong thứ tự tìm việc, và cả hai lần đều có lý do riêng.

```text
 1. lifo_slot            job nóng nhất, vừa spawn xong, cache còn ấm
 2. ring local           rẻ, không tranh chấp
 3. LANE QUEUE           ◀── job từ ngoài vào, và job đã bị spill
 4. steal từ sibling     đắt: phải CAS vào head của người khác
 5. LANE QUEUE lần nữa   ◀── kiểm lại ngay trước khi ngủ
 6. park
```

**Vì sao bước 3 đứng trước bước 4?** Vì lane queue rẻ hơn steal. Bước 4 phải chọn nạn nhân ngẫu
nhiên, CAS vào `head` của họ, có thể thua rồi thử lại, và có thể nhận về `Steal::Busy` khi một
thief khác đang dở tay. Lane queue chỉ có một điểm để hỏi.

Ngoài ra job trong lane queue thường **già hơn**, và job già hơn nên chạy trước.

**Vì sao có bước 5?** Đây là chỗ tinh tế. Giữa lúc worker làm xong bước 3 và lúc nó thực sự park,
main thread có thể vừa push một job vào lane queue. Nếu main thread kiểm "có worker nào đang tìm
...
## 5. Fairness: vì sao phải ép worker ngó lane queue
...
Trong khi đó job của main thread nằm trong lane queue chết đói. Không phải chậm, mà là chết đói:
...
{
    // ép ngó lane queue trước, kể cả ring local đang đầy việc
...
Ring đánh đổi tính linh hoạt lấy tốc độ. Lane queue đánh đổi tốc độ lấy tính không bao giờ từ chối.
Ép một cấu trúc gánh cả hai thì bạn được một cấu trúc dở ở cả hai.

Cụ thể hơn, nếu dùng ring làm lane queue:

- **Nhiều người ghi**: `push` phải thành CAS loop, mất luôn cái `store(Release)` một nhịp.
- **Có biên**: bạn quay lại đúng lỗ hổng 2, chỉ là dời nó lên một tầng. Lane queue đầy thì spill
  đi đâu?
...
[`src/utils/queue_batching.rs`](../src/utils/queue_batching.rs) **chính là một lane queue**, phiên
...
1. Worker `Background` giành được khoá lane queue.
...
- **Mỗi lane một hàng đợi riêng.** Đây là hướng đã chọn, và cũng là lý do kiểu này tên
  `LaneQueue`. Đằng nào các lane cũng không chia việc cho nhau, nên chỉ còn tranh chấp giữa các
  thread cùng priority.
...
Cứ 61 vòng thì ép ngó lane queue trước, kể cả ring local đang đầy việc.

**Dấu hiệu lane queue đang sai**

| triệu chứng | nguyên nhân thường gặp |
|---|---|
| lane queue lúc nào cũng đầy | ring local quá nhỏ, hoặc worker bị bỏ đói |
